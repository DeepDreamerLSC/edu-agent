...
def _stream_requests(fake: FakeLegacy) -> list[dict]:
    return [r for r in fake.requests if r["path"].endswith("/messages/stream")]
...
def test_idempotency_keys_are_stable_across_runs(pool_file):
    ...
    keys_per_run = []
    for _ in range(2):
        with legacy_env(["回复一", "回复二", "完成"]) as (fake, adapter):
            adapter.run_case({"id": "stable-case", "question_id": "q", "student_turns": ["1", "2", "3"]})
            keys_per_run.append([r["body"]["idempotency_key"] for r in _stream_requests(fake)])
...
def test_resume_skips_committed_turns(pool_file):
    ...
    """
    with legacy_env(["t1 的讲解", "t2 的讲解"], preset_committed=1) as (fake, adapter):
        transcript = adapter.run_case({"id": "stable-case", "question_id": "q",
                                       "student_turns": ["1", "2", "3"]})
        stream = _stream_requests(fake)
...
def test_full_flow_drives_dialogue_to_terminal(pool_file):
    with legacy_env(["12个圆片,平均分成4份,每份几个?", "对!每份3个。", "你讲完了,确认结束?"]) as (fake, adapter):
...
def _assert_eval_headers(fake: FakeLegacy) -> None:
...
def test_student_pool_rotates_and_is_stable_per_case(pool_file):
    ...
    with legacy_env(["下一问", "很好,结束"]) as (fake, adapter):
...
def test_stream_error_event_is_environment_failure(pool_file):
    ...
    with legacy_env(["不该到达"], stream_error_on_turn=3) as (fake, adapter):
...
def test_missing_pool_env_is_environment_failure(monkeypatch):
