"""备份接线哨兵(治理④:launchd 定时备份 + 14 天滚动;派单 backup-plist,2026-09-16)。

断言守住已实测的接线事实,任一被改回即红:

1. 备份 plist 存在、每日定时触发(StartCalendarInterval)、调用 scripts/backup-db.sh;
2. plist 不含任何凭据(真相源只能是 0600 的 ~/.config/edu-agent/oss.env);
3. 脚本本地滚动默认 14 天(治理④;M3 旧默认 7 不得改回),连 .sha256 侧车一起删;
4. 远端滚动选型登记在 deploy/launchd/README.md:OSS 生命周期规则(桶管理员一次性
