...
零真实模型(假上游);断言三护栏各自的 event 形态与干净输出零事件。
...
def test_leak_hit_records_rule_and_original(tmp_path):
    ...
    with kernel_env(tmp_path, [completion(open_json("答案是 x=6。对吗?")), completion(clean)]) as (fake, gateway):
...
def test_clean_output_records_nothing(tmp_path):
    with kernel_env(tmp_path, [completion(open_json("你打算从哪一步开始?"))]) as (fake, gateway):
        turn = start({"text": "解方程 3x+7=25。", "answer": "x=6"},
                     {"grade": "五年级"}, gateway=gateway)
