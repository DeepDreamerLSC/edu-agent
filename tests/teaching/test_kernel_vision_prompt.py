...
def test_start_text_question_skips_vision_branch(tmp_path):
    """纯文本题统一 open:只有 tutor 一次调用(acceptable 恒 true)。"""
    fake = FakeOpenAI([completion(open_json("题目要我们求什么?"))]).start()
    ...
    assert [r["model"] for r in fake.requests] == ["fake-model"]  # 统一 open 一次


def test_start_image_acceptable_with_transcription_proceeds(tmp_path):
    """分支①:统一 open acceptable+transcription → 放行,首问照常。"""
    fake = FakeOpenAI([completion(open_json("我们先确认题意:这道题要我们求什么?",
                                            acceptable=True, transcription="解方程 3x+7=25。"))]).start()
    gateway = kernel_gateway(tmp_path, fake.url)
...
def test_start_image_not_acceptable_fails_closed(tmp_path):
    """分支②:统一 open acceptable=False 且纯图 → fail closed(不采信 reply/steps)。"""
    fake = FakeOpenAI([completion(open_json("不该被采信", acceptable=False))]).start()
    gateway = kernel_gateway(tmp_path, fake.url)
    turn = start({"image": "file:photo-2"}, LEARNER, gateway=gateway)
    gateway.close()
    fake.stop()
    assert turn.state == "failed" and "题图" in turn.text
    assert len(fake.requests) == 1  # 一次统一 open,内核 fail closed
...
def test_start_image_travels_as_multimodal_part_not_text(tmp_path):
    ...
    fake = FakeOpenAI([completion(open_json("题图清晰,先读题。"))]).start()
    gateway = kernel_gateway(tmp_path, fake.url)
...
def test_vision_task_carries_explicit_criteria(tmp_path):
    ...
    fake = FakeOpenAI([completion(open_json("先读题。"))]).start()
    gateway = kernel_gateway(tmp_path, fake.url)
...
def test_text_question_with_rejected_image_degrades_to_text_teaching(tmp_path):
    ...
    不终态;fail-closed 只留给无文字兜底的纯图题。"""
    fake = FakeOpenAI([completion(open_json("我们先看已知条件。", acceptable=False))]).start()
    gateway = kernel_gateway(tmp_path, fake.url)
...
def test_start_image_only_question_fills_transcription_as_text(tmp_path):
    ...
    与泄露护栏对照,且不改调用方入参。"""
    fake = FakeOpenAI([completion(open_json("我们先确认题意:这道题要我们求什么?",
                                            acceptable=True, transcription="解方程 3x+7=25,求 x。"))]).start()
    gateway = kernel_gateway(tmp_path, fake.url)
...
def test_reply_copying_analysis_solution_path_is_intercepted(tmp_path):
    ...
    leak = open_json("先在等式两边同时减去7，得到3x=18；再同时除以3，得到x=6。听懂了吗?")
    fake = FakeOpenAI([completion(leak)]).start()
    gateway = kernel_gateway(tmp_path, fake.url)
    turn = start(dict(QUESTION_WITH_ANSWER), LEARNER, gateway=gateway)
    gateway.close()
