# Session 24/24 — `small_lecturer_teaching_context_shadow_pilot_20_stability_equation_parentheses`
...
**t1** student: 我不知道先算括号还是先除。
    tutor: 我们先回到题目本身:你能说说题目给出的条件里,哪一条和这一步有关吗?
    state: dialogue

**t2** student: 我试着把4乘进去，得到4x加8等于24。
    tutor: 你刚才把4乘进去得到4x+8=24，这一步很对！那接下来，我们想让等式左边只剩4x，应该怎么做呢？
    state: dialogue

**t3** student: 再让两边同时减去8，得到4x等于16。
    tutor: 你刚才说两边同时减8得到4x=16，这一步完全正确！接下来，我们怎么从4x=16得到x的值呢？
    state: dialogue

**t4** student: 最后两边同时除以4，x等于4，代回原式得到24。
    tutor: 你刚才说两边同时除以4，x等于4，代回原式得到24，这一步验证了答案的正确性，非常棒！
...
| 3 | dialogue | 4 | 0 | True | False | None |
| 4 | ready_to_confirm | 5 | 0 | True | False | None |

## 标注区(guide v1.1 §6 表单副本;唯一判据 = 包内 guide v1.1)

Session 标注表 —— Session 编号:24  Reviewer(R1/R2):____  日期:____
