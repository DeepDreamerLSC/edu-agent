...
> **二级** = 教参常见错误。
> **2026-09-10 用户裁定(#130)**:15 条二级种子放 M3——定性 = M3 候选池(未经人审,非 v1 入库内容);
> 入库前置 = 补出处(现 15 条全无出处);M2 不动(不替换现有 8 题任何种子,替换会改 `student_turns` → 基线作废)。
> 标注由「待 WP4 用户确认」改为「待 M3 筛选」。本池保持草稿,终版随 M3 扩题筛选收敛。
> 结构性提示:桶 5(application_table)无一级种子(一级定义要求在库出处),M3 扩题时该桶只能靠二级或题面自证。
...
- **二级(待 M3 筛选)** 以「自身」为参照点描述他人相对位置(左右/前后方位反)。
- **二级(待 M3 筛选)** 数轴/方格图中「列 vs 行」混用(先读列还是先读行)。
...
- **二级(待 M3 筛选)** 分数乘法「先乘后约」与「先约后乘」漏约分,结果不是最简。
- **二级(待 M3 筛选)** 分数除法把「除以一个数」错成「乘以被除数的倒数」或只对被除数取倒数。
- **二级(待 M3 筛选)** 单位「1」找错(整体量/部分量错位)。
...
- **二级(待 M3 筛选)** 周长公式 `2πr` 与面积公式 `πr²` 混用(该求周长用了面积,反之)。
- **二级(待 M3 筛选)** 半圆周长漏加直径(只算了半条弧)。
- **二级(待 M3 筛选)** 圆规两脚距离=半径,却直接当直径用。
...
- **二级(待 M3 筛选)** 单位「1」找错——增长率/成数问题里把「基期量」与「现期量」混为被除数。
- **二级(待 M3 筛选)** 百分数参与运算时忘「除以 100」(68% 直接当 68 用)。

## 桶 5 · application_table(应用表格)

- **二级(待 M3 筛选)** 表格行列读反(读取时拿错列/拿错行,或行标题与数据错位)。
- **二级(待 M3 筛选)** 从表格取关键数据时单位不统一直接运算(如 mL 与 L、km 与 m 混算)。
- **二级(待 M3 筛选)** 只完成表格第一步(取数/第一步运算),漏「求答」的最终结果。
...
- **二级(待 M3 筛选)** 速度/时间图像把「斜率/段」与「总量」混淆(只读曲线形状不读坐标)。
- **二级(待 M3 筛选)** 开放题只给结论不给过程(可判定性落在「核心结论可校验」上,s5)。
