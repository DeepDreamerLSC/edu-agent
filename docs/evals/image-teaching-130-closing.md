...
> **2026-09-10 补丁**:悬线清点表初版只列 3 条,漏计 1 条真悬线(answer-leak 口径分叉,#130 上三次提请仍无裁决留痕)与 2 条已处置线(#18 缺图、judge 波动),本版补齐为 6 条三态全量;§4 结论同步。
> **2026-09-10 裁定合入**:用户已裁定二级种子(15 条放 M3,裁定原文见 §3 末),悬线 3 → 已裁定;池文档 15 处标记随手在本 PR 改「待 M3 筛选」(裁定口径第 4 条「搭下一个 docs PR」,即本 PR)。
...
| 3 | 二级误区种子 | 抽查发现三处与派发词不符(证据见下);**2026-09-10 用户裁定:15 条二级放 M3**(裁定原文见下,选项 (ii) 推迟 + 五条口径) | ✅ 已裁定(owner:用户;标记已随手在本 PR 改「待 M3 筛选」) |
| 4 | **answer-leak 口径分叉**(评测面 `KernelSubject.run_case` 不传 `answer`,内核护栏与生产分叉) | **跨线悬置**:#130 上三次提请裁决——09-09 23:53 Dev C 提请两选项(1 保持现状与 11 场景同口径 / 2 评测面补 `answer` 需重种 11 场景基线)、09-10 00:14 步骤 5 仲裁转请 PM、09-10 01:46 收尾重申「等 PM 裁决」——至今无裁决留痕。Dev C 已按**选项 1** 先行;`image-teaching-baseline-v1.md` §6① 记为已知口径差异(23 号泄露判定可能受影响) | ⚠️ 悬置(owner:PM;不阻塞 #130 收尾,M3 连同 11 场景基线一并重议) |
| 5 | #18 缺图(WP1 对账 29/30) | PM 权威清单裁定「idx 18 缺图,出局」、池=29(#130 评论 09-09 23:35);终集 8 题(#5/#6/#9/#14/#20/#23/#27/#30)全出自 29 池 | ✅ 闭环 |
| 6 | judge 单遍波动(fraction_formula_09 两次跑批 12→8) | 处置已记录:`image-teaching-baseline-v1.md` §6③「该题按区间看」;不改数、不重跑 | ➖ 无后续 |
...
### 悬线 3 裁定(用户,2026-09-10,#130 PM 记录评论)

**裁定:二级种子 15 条放 M3**(实采上文选项 (ii) 推迟,并加五条口径):

1. **定性** = M3 候选池(未经人审),非 v1 入库内容;
2. **入库前置** = 补出处(现 15 条全无出处;"一级"5 条均注明在库出处);
3. **M2 不动** = 不替换现有 8 题任何种子(替换会改 `student_turns` → 基线作废)、不收敛条数;
4. **文档标记** = 池文档 15 处「待 WP4 用户确认」→「待 M3 筛选」,随手搭下一个 docs PR(即本 PR,已改,含定义行同步);
5. **结构性提示** = 桶 5(application_table)无一级种子(一级定义要求仓库既有场景出处),M3 扩题时该桶只能靠二级或题面自证。

> 裁定采用本文修正后的实数(15 条),即上文证据 1 的更正(派发词"16"疑为把定义行计入所致,`grep -c` 全文命中 16 = 15 条种子 + 1 行定义);数据集 7 条"（二级）"标注种子按口径 3 维持 M2 现值(裁定前 PM 建议口径的前提"8 题全是一级"不实,裁定按实际状态作出)。
...
- 三态清点(6 条):悬线 1/2/3/5 ✅ 闭环(3 为已裁定);悬线 6 ➖ 无后续(处置已记录);**仍悬置 1 条**——悬线 4(answer-leak 口径分叉,owner=PM,已按选项 1 先行并记录为已知口径差异,不作为 #130 闭键)。
- **结论**:悬线 3 已裁定(15 条二级放 M3,M2 不动)——**#130 全部工作包与悬线落地,可闭**;闭键在用户。悬线 4 为跨线问题(11 场景同受影响),按 Dev C 选项 1 口径推迟 M3 重议。
