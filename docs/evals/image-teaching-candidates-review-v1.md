...
2. 学生:可是往北800米到底是几个格？一格是200米，我算成4格……和B的纵坐标对不上。
...
1. 学生:先把四个顶点写成数对：我读出来A(1,5)、B(1,2)、C(5,2)、D(3,5)。
2. 学生:向右平移4格，就是把第一个数加4；向下平移2格，第二个数减2。所以A₁应该是(5,3)。
3. 学生:可是B₁我算成(5,4)了，和A₁横坐标一样、纵坐标不一样……我得看看图上平移后顶点到底落在哪。
...
1. 学生:汽车行驶了20分钟吧？图一直画到20分钟那里。
2. 学生:但中间8到10分钟速度降到0了，那段时间车是停着的。行驶时间算不算停着的时间？
3. 学生:最大速度是30千米/时，就是前面那段保持不变的。8到10分钟速度是0，应该是停车了。
