...
用户裁定(c5674864751):**点火(任务是否开始)=用户的键**。用户裁定(2026-09-16 路由改革):**任务内修复循环 dev↔reviewer 直连,PM 不中转;PM 负责派工、跨任务协调、异常裁决、最终收口**。本 skill 固化操作纪律。
...
### 循环内路由(用户裁定 2026-09-16)

| 消息 | 接收者 | PM 立即介入 |
|---|---|---|
| 开发完成,请求审查 | 派单时指定的 Reviewer | 否 |
| 审查要求修改 | 原开发者 | 否 |
| 修复完成,请求复审 | 原 Reviewer | 否 |
| 审查通过,待合并 | PM 汇总给用户 | 是 |
| 超预算、扩范围、验收争议、跨任务冲突 | PM | 是 |
| 正常进度、收到、开始处理 | 留在任务记录(to=log) | 否 |

新增消耗或范围变化仍按既有授权边界升级(点火键在用户)。Reviewer 独立性、用户合并权不变;审查结论保留在 PR,他处只链接。
...
- (脚本跟进·路由改革第二波)按 `to=` 筛选唤醒、**全量有序交付**(现 max_by 单条会吞同波中间评论)、>100 评论分页、d-state 改「仍欠回执且超约定时间」才报警——当前 PM 手工执行,脚本修好后自动化;
...
2. **按接收者分类处理**(2026-09-16 路由改革;循环内消息已由 dev↔reviewer 直连,不靠 PM 转发):
   - `to=pm` 且审查通过/待合并 → 收口:核验 + 汇总报用户(合并键在人);
   - `to=pm` 且异常类(超预算/扩范围/验收争议/跨任务冲突)→ 按冻结口径裁决,留痕于对应 issue/PR;
   - 其余(`to=reviewer/dev/log`)→ **不中转、不重复派发**(试行验收点);PM 留痕评论后同步 `issue-watch.state` 到该评论 id(避免自触发);
3. **按冻结口径核验**(判卷/判读标准在派单或 #253 冻结评论里,跑后不改)——仅对 PM 介入面;
4. 裁定:收编/驳回/追问——追问可直发;
5. **审查派发**:开发者任务的审查者已在派单时指定(开发完成回执直达审查者);PM 自己开的 PR 仍由 PM 派审(659c6ae9,queue;已派记录入 `review-dispatched.txt` 防重);审查零 API,无需点火;
6. **点火级/红灯才报用户**(候选升级、基线落定、门红诊断、审查通过待合并);其余自动消化。
...
- **中转任务内修复循环**(2026-09-16 路由改革):审查修改单直达原开发者,修复回执直达原审查者;PM 只做派工/跨任务协调/异常裁决/收口。
