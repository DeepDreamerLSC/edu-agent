...
2. **方案 ponytail 自审**(强制,不可跳过):派单前对方案跑一次 `ponytail-review`,确认是最短方案、无伪选项、无重复 boilerplate、无 over-engineering。自审发现的问题**先修再派**,不派半成品;
3. **写消息**:首行签名 `【PM 7e9b92bc 直发 YYYY-MM-DD】`;任务块自包含(基底指纹/输入/跑法/判读预注册/工件/回执要求/纪律);按「先快后长」排序逐条发;
...
6. **重挂看门狗**(第二节);若自己在被盯 issue 发了评论,先把
   `issue-watch.state` 中该 issue 的 id 同步到该评论 id 再重挂(避免自触发唤醒);
7. **点火级/红灯才报用户**(候选升级、基线落定、门红诊断);其余自动消化。
