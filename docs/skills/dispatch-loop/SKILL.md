...
**主树 `/root/code/edu-agent` = 只读锚树**;**唯一合法执行现场 = 任务专属 worktree**。并行开发是常态(load-rebalance/night-0918 先例:3-4 会话同时在飞),「主树=当前活跃者」需要实时占用仲裁而 dsh 会话间无文件锁,故不设主树专属权,设主树恒净不变量。

| | 主树(锚) | 任务 worktree |
|---|---|---|
| 身份 | main 的本地只读镜像 | 唯一合法执行现场 |
| 允许 | `git fetch` / `git pull --ff-only`(main 追平远端)/ 只读命令(log/diff/status/gh) | 全部操作,含 commit、make check、建 .venv |
...
- **终态检验(代替锁)**:任何时刻任何会话看主树 `git status` 非净 = 有违规或遗留 → 升级 PM 追责。无主未跟踪物(如 calibration-kit/)不算违规,单独归属处置;
- **过渡**:规则生效时已在主树进行的在飞任务(存量例外)可干到收口,收口即迁;此后新任务一律 worktree;
- **成本**:uv in-project venv(~99MB/树)+ 各树各跑 make check,既有 88 树实践证明可付;
- **卫生**:任务 close 后即删树;PM 定期 `git worktree prune` + 清 `/tmp` 临时树(大量对应已 ✅ 任务)。
...
3. **写消息**:首行签名 `【PM <session-id> 直发 YYYY-MM-DD】`;任务块自包含(基底指纹/输入/跑法/判读预注册/工件/回执要求/纪律);按「先快后长」排序逐条发;
   ...
   - **回执要求写进派单**:回执首行加机器可读头 `<!--RECEIPT task=<id> pr=<n|-> calls=<n> tier=<flash|pro|0> outcome=<...> to=<reviewer|dev|pm|log> head=<git-sha8> dev=<会话id> wt=<worktree路径> sha256=<hex16>-->`(渲染不可见),并回显派单摘要值;`to=`=接收者(共用 GitHub 账号,不能按作者过滤;看门狗按此筛选唤醒,普通进度 `to=log` 不唤醒),`head=`=代码身份,`dev=`=开发者会话身份(必填,自报家门;共用 GitHub 账号下区分回执提交者,**审查者见缺即退**),`wt=`=工作现场(见「零、工作现场」;执行类任务必填,审查/纯读任务可省);
     - **v3.1(2026-09-17)**:增 `dev=` 必填字段。变更理由:路由争议 c5707343575——#310 链回执无自报家门、共用 GitHub 账号不可区分,致修改单路由失败实害;PM-RULING#6① 流程修补即刻生效落成文。
     - **v3.2(2026-09-20)**:增 `wt=` 字段 + 「零、工作现场」节。变更理由:30 会话共用主树——stash 遗留(git stash@{0} 2026-09-17)、分支横跳(reflog 单日 5 次 checkout 切换)两起实害;「每任务一 worktree」实践已存在(88 树)但未成文,主树仍是默认落点。用户批准 2026-09-20。
...
**完成仪式两步(缺一不可,PR 开了 ≠ 完成)**:①回执落派单指定位置(首行含 `to=`/`head=`);②`dsh-rpc.py send <审查者子串> queue "<task-id> 完成,回执已落 <位置>,请审查;**我是 <开发者标识>,修改单请发我**;派单摘要 sha256=<回显值>"`——审查者会话**没有看门狗**,不 queue 它就不知道;**queue 消息必须自报家门**(共用 GitHub 账号,回执按作者分不出谁是谁;2026-09-16 实测:①PR #300 开了无回执无通知,循环挂起;②修改单因无自报家门被误路由到非原开发者,靠 PM 纠正)。PM 派单模板必须把这两步写全。
...
| 主树 status 非净 | 先 `dsh-rpc.py list` 查谁在跑——在跑会话涉主树 = 违反「零、工作现场」,升级 PM 裁置;无人认领 = 遗留物,挂起处置(先例:stash@{0} 2026-09-17 他人遗留) |
...
- **在主树执行写操作**(2026-09-20 工作现场裁定):checkout/commit/stash/reset/clean 一律在任务 worktree;主树只读。存量例外见「零、工作现场」过渡条款。
