...
**完成仪式两步(缺一不可,PR 开了 ≠ 完成)**:①回执落派单指定位置(首行含 `to=`/`head=`);②`dsh-rpc.py send <审查者子串> queue "<task-id> 完成,回执已落 <位置>,请审查"`——审查者会话**没有看门狗**,不 queue 它就不知道(2026-09-16 实测:PR #300 开了无回执无通知,循环挂起;PM 派单模板必须把这两步写全)。
