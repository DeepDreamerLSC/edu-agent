...
description: >
  ...
  依赖、六色状态语义(含↷延期/冻结)、事实前置核验。Trigger: 「更新进展图」「重出进展图」
...
- **工件落盘位置**:私有工件目录(如 `calibration-private/`)或评审包目录,
  **不进本仓库**——图内含 issue/PR 号与进行中状态,且可能嵌未脱敏内容;
  文件名约定 `<里程碑>进展图.html`;
- **同步纪律**:语义级变更(如状态色/口径规则)先改运行正本,再同步本仓副本,
  两处不得各自漂移。
...
2. **legend 六色**(全局唯一状态语义;2026-09-16 从五色升六色):
   ✓ 完成 `#16a34a`(**口径=已合并/正式生效;审查可合≠Done**) ·
   ⏳ 在途(已派单/在跑/已核验待合并) `#2563eb` ·
   🔑 等用户的键(只缺人裁) `#d97706` · ⏸ 阻塞 `#dc2626` ·
   ○ 未开始 `#9ca3af` ·
   ↷ 延期/冻结(有触发条件,紫色 `#7c3aed`)——**延期不计入「未完成」**;
3. **总览 = 全景线框**(核心区块):
...
- **✓ 只计已合并/正式生效**——PR 开着哪怕审查可合也是 ⏳(注「待合并键」);
- 未合并的机制/工件不因「核验过」升 ✓,可注「已核验待生效」;
- **阶段完成度优先用可数子门表达**(`2/4 gate 已核验`、`机制已审/首跑未发生`、`3/6 前置确认`),
  % 仅作进度条视觉、标签必须可对着回执数出来——结构性反伪装,不只靠口头披露;
- **百分比 = PM 判断值**,不是测量值——交付时必须口头声明一句
  (「各阶段 % 是判断值,定性看状态点更准」),图内不伪装;
- 计数行与 items 状态点必须自洽(数得出来)——标题分数也要数得出来;
- 键清单按 **lead-time/关键路径影响** 排序,不按发生顺序。
