...
benchmark 门分诊纪律（#241 行1，标志位让路）：门红了**先读报告里的争用证据块再判回归**——
证据块显示无申报批跑且模型服务 CPU 低，按真回归走 30 分钟纪律。见到 SKIPPED（批跑窗口
让路）不算红：批跑收工后 `gh run rerun <run> --job <benchmark-job>` 补跑即可。
