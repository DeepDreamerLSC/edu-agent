...
- 分支活不过 **一天**。CI 检查 PR 首个提交距今时间，超过打 `stale-branch` 标签并阻止合并，需 rebase。
- 同时打开的 PR **不超过两个**，agent 并行开工上限也是两个。
...
- [ ] GitHub 分支保护：CI 必须通过 + Require branches to be up to date；不开必需评审
- [ ] `.github/workflows/main.yml`：合并后评测冒烟 + 自动部署 test
