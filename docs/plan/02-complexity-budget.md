...
| 规则可豁免等于没有规则 | 2130 行文件上限"不得提高"，agent.py 4117 行靠棘轮基线豁免存活 |
| 没有用户却按生产系统建 | 1.1 万行蓝绿发布测试、87 个迁移、多租户可见性策略、运行时身份契约 |
| 自建基础设施 | worker 池、租约、心跳、进程内 MLX 引擎、6 种 provider；300 次提交中 33 次在修它们 |
| 测试镜像实现 | 测试 41.7 万行，多于应用代码 42.9 万行；141 个测试文件耦合 lease 实现，删代码比加代码贵 |
