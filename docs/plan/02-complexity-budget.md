...
要加就先删。唯一例外是测试比（见第 6 节）：**永久只报告不阻塞**——它是可被搬运刷低的代理指标
（把 helper 搬进 `tests/fixtures/` 即降分子），给可刷指标设门会重演 #142（门必须建在可复现、
不可刷的量上）。2026-09-11 人裁，撤回原"M2 起阻塞"的分期计划。

行数类指标只统计 `*.py` 文件，**扫描范围为全仓库**（`edu_agent/`、`scripts/`、`tests/` 都算——
严不会错，实现即 budget.py 现状）；docstring 属非注释行，**计入行数**（偏保守，刻意选择）。
数据集、artifacts、fixture 等非 Python 文件不计入总行数与单文件行数（配置文件数量照计），
否则一个上千行的 JSONL 数据集就会撞 800 行单文件上限。
...
| 配置文件数量（`configs/`） | 5 | 同上 |
| 测试代码 / 应用代码 行数比 | ≤ 1.0 | 同上；**永久只报告不阻塞**（见第 6 节，2026-09-11 人裁） |
| 部署脚本行数 | 100 | 同上 |
...
- 测试与应用代码行数比不超过 1.0（见第 2 节）。分母是 `edu_agent/`，分子**不含** `tests/rules/` 与 `tests/fixtures/`
  （假上游服务器等测试基础设施）。该比值**永久只报告不阻塞**（2026-09-11 人裁）：分子可被"把 helper 搬进
...
3. **agent 只开 PR，永远不合并**是铁律。同一账户下没有技术手段阻止 agent 按合并键，
   防线是 AGENTS.md 的规矩与 PR 时间线的可回溯性；若需要技术强制：升级 GitHub Pro 开
   分支保护，并引入 GitHub App 给 agent 降权（App 可推分支、开 PR，但被分支保护挡在
   合并之外），不在 v1。
