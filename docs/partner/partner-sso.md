...
> **v1 范围说明**(新后端,2026-09-08;吊销注记 2026-09-21 更新,设计稿 #347):本文档基于
> 合作方合同原样迁移。v1 后端**支持**:原生授权码 + PKCE 全舞步
> (`POST /api/openapi/v1/auth/native-codes` → `POST /api/auth/native/token`),单合作方。
> v1 后端**支持**退出(`POST /api/auth/logout`):返回 `{"ok": true}` 且**立即吊销当前
> Token**(jti 吊销名单,§7.1)——共享设备轮用的合同义务由此兑现;缺头/验签失败也返回
> 200 `{"ok": true}`(幂等空操作,客户端视角「登出总是成功」)。v1 后端**不支持**:多合作方、
> 刷新令牌(`refresh_token`)、网页 Cookie SSO(自有网页单点登录)、管理端吊销他人 token、
> 登出广播/踢下线。
...
| `tenant_id` | 是 | 当前学生所属学校 | 示例字段名；实际字段名和值由双方联调时确定。 |
