...
5. 总结与掌握判断(summary_mastery):
   ...
   **注:**总结中复述学生已经给出的答案不因复述本身扣分,也不属于终答泄露;"注意事项"等补充内容可以作为额外证据,但不能替代对学生实际学习证据和掌握状态的判断。
   总结中声称的『学生证据/学生理解』须与对话实际交叉核对:若对话显示该关键内容实为小讲师所给(学生并未自己得出),不得按面值采信为学生本人关键学习证据——总结把它记到学生头上时,按掌握判断缺依据降档。
...
实质性门槛不因枚举而放宽:标注为错但不影响解题结论或方法的局部不精确,记入 evidence 但不单独触发 0。

**0 分**:学生把坐标两个分量互换,小讲师仍称"这一步很准";学生最终结论 `2/3 × 6 = 4` 正确、但中间推理有误,小讲师没有保留正确结论,反而总结称"结果才是 2,而不是 4";学生把圆面积错误表述为"半径 × 3.14",小讲师复述该完整方法并称"这个想法很对";小讲师自身编造了错误的分母运算、错误公式或其他会改变解题结果/方法的数学事实。
