"""#253 v3.1 文本收口:同 7 案 ds 重跑(件 3 收窄 + 改话锚点 + 件 2 强制引用)。
...
输出原样落盘;judger_sha256(v3.1 指纹)随产物记录随产物记录。
"""
import json
import shutil
import sys
...
# 自含复算:输入复用 judge-v3-pilot-7 存档(逐字);输出/facts 落 /tmp。
# 需 DEEPSEEK_API_KEY。
CASES_FILE = Path(__file__).resolve().parent.parent / "judge-v3-pilot-7" / "judge-cases.jsonl"
OUT_DIR = Path("/tmp/pilot7-v31-rerun")
FACTS = Path("/tmp/pilot7-v31-rerun-facts")
