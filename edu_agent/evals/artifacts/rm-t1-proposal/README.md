...
- `pairs.jsonl`:8 组候选对(每组 = A/B 两段逐字引文 + 差异说明 + RM 判别力一句);
  PM 逐组裁定(c5678418082,2026-09-16):**8 留 1 裁**——T1-P04 剔除(A 侧错误
  在片段内不可见,配对退化无探测力),其余 8 组全留;
...
错误类型覆盖:错误等值×4 / 假规则×2 / 归因错位×1 / 慈善转述×1(P02 兼归因与慈善,
P09 为纯慈善转述)。

## 配对方法与诚实账

- **配对判定是人工的,引文抽取是机械的**:轮次区间人工选定,引文由脚本从源 JSONL
  按 (case_id, 轮次下标) 抽取,无手抄(裁定剔除 P04 后 45/45 轮与源逐字节一致,
  见上复跑命令);
- **同文优先**:6 组跨案同题配对(同题面同位置同话步),2 组同案最小对比(T1-P03
