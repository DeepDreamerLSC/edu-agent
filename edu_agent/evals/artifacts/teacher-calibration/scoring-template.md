...
|---|---|---|---|---|---|
| 1 | C40 | 数学真实性 |  |  | expected **0**;终冻基线 **0/2 跑间分裂**(已知限:规则命题被计算命题吸收;两跑不一致保守端计 fail) |
| 2 | C26 | 数学真实性 |  |  | expected **2**;终冻基线 **2/2**(正向位:误念修复,健康锚) |
| 3 | C25 | 学生掌握归因 |  |  | expected **1**(sm≥1);终冻基线 **0/0**(边界位:方法总结在/学生证据缺) |
| 4 | C11 | 转述忠实性 |  |  | expected **0**;终冻基线 **2/2**(已知限:转述错位被合理化) |
| 5 | C36 | 转述忠实性 |  |  | leak 期望 **False**;终冻基线 **False/False**(正向位:转述忠实捡学生思路不补终答;leak 不经教师;替代 C35,#310 人裁) |
